  ...
  <VTable>
    ...
    <v-slide-y-transition :group="true" tag="tbody" :leave-absolute="true" :hide-on-leave="true" :duration="750">
      ...
      >
        ...
        <td v-if="isHeaderVisible('contactsCount')" class=" base-table__td">
          {{ item.contactsCount }}
        </td>
        ...
        <td class="text-center" v-if="!props.hideActions  && isHeaderVisible('actions')">
          <div class="d-flex align-content-center">
            <VBtn
              ...
              height="25"
              color="info"
              @click.stop="emit('click:add-contacts', item)"
            >
              <VIcon icon="bx-plus"/>
              <VTooltip
                activator="parent"
              >{{ $t('data.title.addInContacts') }}
              </VTooltip>
            </VBtn>
            <VBtn
              icon
              ...
              @click.stop="handleChangeStatus({id: item.id, status: item.status === STATUSES.ACTIVE ? STATUSES.ARCHIVE: STATUSES.ACTIVE})"
            >
