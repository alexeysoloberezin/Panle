...
const chartOptions = {
  ...
  plugins: {
    title: {
      display: true,
      text: 'Распределение по таймзонам'
    },
    tooltip: {
      callbacks: {
...
const fetchRequest1 = async () => {
  ...
  const chartD = TimeZone.getData(date.value, props?.timeRange, props?.userId)

  const {error, messageError, data} = await TimeZone.fetchData(chartD, true)
