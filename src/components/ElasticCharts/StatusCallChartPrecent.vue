...
import StatusCallsChart from "@/js/Elastic/variants/StatusCallsChart";
...
const chartOptions = {
  ...
  plugins: {
    title: {
      display: true,
      text: 'Статусы по звонкам в процентах %'
    },
    tooltip: {
      callbacks: {
...
const fetchRequest1 = async () => {
  ...
  const chartD = StatusCallsChart.getData(date.value, props?.timeRange, props?.userId)

  const {error, messageError, data} = await StatusCallsChart.fetchData(chartD, true)
