...
import ColAnswerChart from "@/js/Elastic/variants/ColAnswerChart";

const props = defineProps(['url', 'date','userId', 'timeRange', 'minH'])
...
const fetchRequest1 = async () => {
  ...
  const chartD = ColAnswerChart.getData(date.value, props?.userId)

  const {error, messageError, data} = await ColAnswerChart.fetchData(chartD)
