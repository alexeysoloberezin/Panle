...
import ErrorDetectChart from "@/js/Elastic/variants/ErrorDetectChart";
...
const fetchRequest1 = async () => {
  ...
  const chartD = ErrorDetectChart.getData(date.value, props?.timeRange, props?.userId)

  const {error, messageError, data} = await ErrorDetectChart.fetchData(chartD)
