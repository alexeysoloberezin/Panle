<template>
  <v-card v-if="Array.isArray(date)" class="mt-3 pa-3" >
    <div v-if="!message">
      <Bar :type="'line'" :data="chartData" :options="chartOptions" :height="heightProp"/>
    </div>
    <div v-else>
      {{ message }}
    </div>
...
const chartOptions = {
  ...
  plugins: {
    title: {
      display: true,
      text: 'Скорость детекции'
    },
    tooltip: {
      callbacks: {
        label: function (context) {
          const value = context.dataset.data[context.dataIndex];
          return value.toFixed(4) + ' /сек';
        },
      },
    },
    zoom: {
...
const fetchRequest1 = async () => {
  ...
  CPSChart.calculateInterval(date.value, props?.timeRange)

  const chartD = CPSChart.getData(date.value, props?.timeRange, props?.userId)

  const {error, messageError, data} = await CPSChart.fetchData(chartD, date.value, props?.timeRange)
