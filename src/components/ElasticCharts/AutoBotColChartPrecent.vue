...
const chartOptions = {
  ...
  plugins: {
    title: {
      display: true,
      text: 'Кол-во автоботов и АО в процентах'
    },
    tooltip: {
      callbacks: {
...
const fetchRequest1 = async () => {
  ...
  const chartD = AutoBotColChart.getData(date.value, props?.timeRange, props?.userId)

  const {error, messageError, data} = await AutoBotColChart.fetchData(chartD, true)
